...
def densenet121(growth_rate=32, compression=1.0):
    ...
    bacth0 = Keras.layers.BatchNormalization()(X)
    relu0 = Keras.layers.Activation('relu')(bacth0)

    conv1 = Keras.layers.Conv2D(filters=2*growth_rate, kernel_size=7,
                                strides=2, padding='same',
                                kernel_initializer=init)(relu0)

    max_pool1 = Keras.layers.MaxPooling2D(pool_size=3, strides=2,
                                          padding='same')(conv1)
    ...
    avg_pool = Keras.layers.AveragePooling2D(
        pool_size=7, padding='same')(dense4)
