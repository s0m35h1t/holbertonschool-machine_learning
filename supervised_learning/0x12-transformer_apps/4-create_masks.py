...
def create_masks(inputs, target):
    ...
    Returns: encoder_mask, look_ahead_mask, decoder_mask
            ...
            decoder_mask is the tf.Tensor padding mask of shape
                ...
                the decoder
    """
    encoder = tf.cast(tf.math.equal(inputs, 0), tf.float32)[
        :, tf.newaxis, tf.newaxis, :]

    decoder = tf.cast(tf.math.equal(inputs, 0), tf.float32)
    decoder = decoder[:, tf.newaxis, tf.newaxis, :]
    ...
    combined = tf.maximum(
        dec_target_mask, 1 - tf.linalg.band_part(tf.ones((size, size)), -1, 0))
    return encoder, combined, decoder
